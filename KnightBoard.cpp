...
class KnightBoard {
    ...
    Pose KnightBoard::getIndexFromPosition(const Pose &pose) {
        return size_* pose.x + pose.y;
    }

    vector<vector<int>> KnightBoard::getCostMatrix(){
      vector<vector<int>> adjacency_matrix(size_*size_, vector<int>(size_*size_, INT_MAX));
      for(int row = 0; row < size_* size_; row++) {
        for(int column = 0; column < size_* size_; column++) {
                    ...
                    case BarrierType::TELEPORT:
                        ...
                        for (auto tile_pose: getTeleportTiles()) {
                            if (tile_pose == end_pose) {
                                adjacency_matrix[pose.x][pose.y] = INT_MAX;
                            }
                            else {
                                adjacency_matrix[pose.x][pose.y] = 1;
                            }
                        }
                        break;
                }
    ...
    vector<Pose> KnightBoard::findPathHelper(vector<int> &parent, Pose &end_pose) {
        ...
        printPath(parent, end_pose_index);
        vector<Pose> path;
        while(true){
